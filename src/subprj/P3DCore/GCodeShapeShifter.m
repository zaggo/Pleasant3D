...
//  Copyright 2012 Pleasant Software. All rights reserved.
//
//
...
#import "GCodeShapeShifter.h"
#import "Vector3.h"

enum { kRotateX, kRotateY, kRotateZ };

static void rotateVertex(NSInteger axis, Vector3* p, float angle)
{
	float cos = cosf(angle);
	float sin = sinf(angle);
	
	float inP[3] = {p.x, p.y, p.z};
...
@implementation GCodeShapeShifter
@synthesize sourceGCodeString, objectRotateX, objectRotateY, objectRotateZ, processedGCodeString, objectScale, dimX, dimY, dimZ, centerX, centerY, originX, originY, originZ, undoManager;
...
- (id)initWithCoder:(NSCoder*)decoder
{
	...
	if(self)
	{
		...
		originX = [decoder decodeFloatForKey:@"originX"];
		originY = [decoder decodeFloatForKey:@"originY"];
		originZ = [decoder decodeFloatForKey:@"originZ"];
	}
	return self;
}
...
- (void)encodeWithCoder:(NSCoder*)encoder
{
	...
	[encoder encodeFloat:originX forKey:@"originX"];
	[encoder encodeFloat:originY forKey:@"originY"];
	[encoder encodeFloat:originZ forKey:@"originZ"];
}

+ (NSSet *)keyPathsForValuesAffectingDimX {
    return [NSSet setWithObjects:@"processedGCodeString", nil];
}
...
- (void)processGCodeString
{
	[self willChangeValueForKey:@"processedGCodeString"];
	processedGCodeString=nil;
	
	if(sourceGCodeString)
	{
		processedGCodeString = [sourceGCodeString copy];
...
//		processedSTLModel.cornerMinimum.z-=processedSTLModel.cornerMinimum.z-minZ;		
	...
	[self didChangeValueForKey:@"processedGCodeString"];
}

- (void)setSourceGCodeString:(NSString*)value
{
	if(sourceGCodeString!=value)
	{
		sourceGCodeString=value;
		[self processGCodeString];
	}
}
...
- (void)setOriginX:(CGFloat)newOriginX
{
	if(self.originX!=newOriginX)
	{
		[[undoManager prepareWithInvocationTarget:self] setOriginX:self.originX];
		originX = newOriginX;
		[self processGCodeString];
	}
}
...
- (void)setOriginY:(CGFloat)newOriginY
{
	...
	{
		[[undoManager prepareWithInvocationTarget:self] setOriginY:self.originY];
		originY = newOriginY;
		[self processGCodeString];
	}
}

- (void)setOriginZ:(CGFloat)newOriginZ
{
	if(self.originZ!=newOriginZ)
	{
		[[undoManager prepareWithInvocationTarget:self] setOriginZ:self.originZ];
		originZ = newOriginZ;
		[self processGCodeString];
	}
}
